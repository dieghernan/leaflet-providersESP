...
// BSD 2-Clause "Simplified" License
 function LPESP_composenames (arg,
	...
	if (variantName && 'variants' in
		providers[providerName]) {
		if (!(variantName in providers[
				providerName].variants)) {
			throw 'No such variant of ' +
				providerName + ' (' + variantName +
				')';
		}
		var variant = providers[providerName]
			.variants[variantName];
		//Guess WMS or WMTS
		var finalurl = variant.url ||
			provider.url;
		...
		provider = {
			...
			options: L.Util.extend({},
				provider.options, variantOptions
			)
		};
	}
		...
		return attr.replace(
			...
			function (match, attributionName) {
				return attributionReplacer(
					providers[attributionName].options
					.attribution);
			}
		);
	};
	provider.options.attribution =
		attributionReplacer(provider.options
			.attribution);
	// Compute final options combining provider options with any user overrides
	var layerOpts = L.Util.extend({},
...
L.tileLayer.providerESP = function (
	...
	if (nameurl.includes("{x}")) {
		return L.tileLayer(newprov.url,
			newprov.options);
	} else {
		return L.tileLayer.wms(newprov.url,
			newprov.options);
	}
};